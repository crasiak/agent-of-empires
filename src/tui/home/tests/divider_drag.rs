...
#[test]
#[serial]
fn drag_follows_pointer_clamps_and_persists_once() {
    let mut env = create_test_env_empty();
    stage_side_by_side(&mut env);
    assert!(!env.view.handle_drag_move(50, 5), "move without start");
    ...
    // Far left of column 0: the i32 math must absorb the negative without wrapping u16.
