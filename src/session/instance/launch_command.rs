...
impl Instance {
    ...
    pub(super) fn refresh_prepared_prime_launch_after_pane_stop(
        ...
        // The rebuild unsets the restart intent; keep the one recorded before teardown.
        let ledger_restart_env = prepared
            .launch_env
            .pane
            .iter()
            .filter(|mutation| {
                matches!(mutation, tmux::PaneEnvMutation::Set { key, .. }
        ...
        refreshed.launch_env.pane.extend(ledger_restart_env);
        Ok(refreshed)
    }
